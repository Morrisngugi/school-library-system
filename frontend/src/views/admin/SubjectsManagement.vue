  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="md:flex md:items-center md:justify-between mb-6">
      <div class="flex-1 min-w-0">
        ...
        <p class="mt-1 text-sm text-gray-500">Manage book subjects and classifications</p>
      </div>
          ...
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                ...
                <dd class="text-lg font-medium text-gray-900">{{ subjects.length }}</dd>
              </dl>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white overflow-hidden shadow rounded-lg">
        <div class="p-5">
          <div class="flex items-center">
            ...
            <div class="ml-5 w-0 flex-1">
              <dl>
                <dt class="text-sm font-medium text-gray-500 truncate">Active Subjects</dt>
                <dd class="text-lg font-medium text-gray-900">{{ activesubjects }}</dd>
              </dl>
            </div>
          </div>
        </div>
    ...
    <div v-else-if="subjects.length === 0" class="bg-white shadow rounded-lg p-8 text-center">
    ...
    <div v-else class="bg-white shadow overflow-hidden sm:rounded-lg">
      <ul class="divide-y divide-gray-200">
        <li v-for="subject in subjects" :key="subject._id" class="px-6 py-4 hover:bg-gray-50">
          <div class="flex items-center justify-between">
            <div class="flex-1 min-w-0">
              <div class="flex items-center space-x-3">
                <span class="inline-flex items-center justify-center h-10 w-10 rounded-lg" :style="{ backgroundColor: subject.color || '#6366f1' }">
                  <span class="text-white font-semibold text-sm">{{ subject.code }}</span>
                </span>
                <div>
            ...
            <div class="flex items-center space-x-4">
              <div class="text-right">
                <p class="text-sm font-medium text-gray-900">{{ subject.bookCount || 0 }} books</p>
                <p class="text-xs text-gray-500">
                  <span :class="subject.isActive ? 'text-green-600' : 'text-red-600'">
                    {{ subject.isActive ? 'Active' : 'Inactive' }}
                  </span>
                </p>
              </div>
              <div class="flex space-x-2">
                <button @click="editsubject(subject)" class="text-indigo-600 hover:text-indigo-900">
                  <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                  </svg>
                </button>
                <button @click="deletesubject(subject._id)" class="text-red-600 hover:text-red-900">
                  <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              ...
              <div class="space-y-4">
                ...
                <div>
                  ...
                  <textarea v-model="formData.description" rows="3" placeholder="Brief description of the subject" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
                </div>
                ...
                <div class="flex items-center">
                  ...
                  <label class="ml-2 block text-sm text-gray-900">Active subject</label>
                </div>
              </div>
            </div>
...
const activesubjects = computed(() => subjects.value.filter(c => c.isActive).length)
const totalBooks = computed(() => subjects.value.reduce((sum, c) => sum + (c.bookCount || 0), 0))
...
async function fetchsubjects() {
  ...
  try {
    ...
    const errorMsg = error.response?.data?.error || error.message || 'Failed to load subjects. Please try again.'
...
async function savesubject() {
  ...
  try {
    if (editingsubject.value) {
      await axios.put(`/api/v1/catalog/subjects/${editingsubject.value._id}`, formData.value)
    } else {
      await axios.post('/api/v1/catalog/subjects', formData.value)
    }
    closeModal()
    fetchsubjects()
    alert(`subject ${editingsubject.value ? 'updated' : 'created'} successfully!`)
  } catch (error) {
...
async function deletesubject(subjectId) {
  ...
  try {
    await axios.delete(`/api/v1/catalog/subjects/${subjectId}`)
    fetchsubjects()
